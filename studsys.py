...
def export_data():
    ...
    #table=panda.DataFrame(newlist,columns=['admission no','Name','EMIS no','date of birth','house','Gender','new or old','Added Date','Added father occupation'])
    #table.to_csv(url,index=False)
    messagebox.showinfo('Success','Data is saved succesfully')


def toplevel_data(title,button_text,command):
    global nameEntry,emisnoEntry,emailEntry,genderEntry,fathernameEntry,fatheroccupationEntry,fatherannualincomeEntry,officeadressEntry,fatherphonenoEntry,mothernameEntry,motherphonenoEntry,motheroccupationEntry,motherannualincomeEntry
    global screen
    screen = Toplevel()
    screen.title(title)
    screen.grab_set()
    
    idLabel = Label(screen, text='admission no', font=('times new roman', 12, 'bold'))
    idLabel.grid(row=0, column=0, padx=6, pady=2, sticky=W)
    idEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    idEntry.grid(row=0, column=1, pady=2, padx=2)
    
    nameLabel = Label(screen, text='Name', font=('times new roman', 12, 'bold'))
    nameLabel.grid(row=1, column=0, padx=6, pady=2, sticky=W)
    nameEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    nameEntry.grid(row=1, column=1, pady=2, padx=2)

    emisnoLabel = Label(screen, text='emis no', font=('times new roman', 12, 'bold'))
    emisnoLabel.grid(row=2, column=0, padx=6, pady=2, sticky=W)
    emisnoEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    emisnoEntry.grid(row=2, column=1, pady=2, padx=2)

    emailLabel = Label(screen, text='date of birth', font=('times new roman', 12, 'bold'))
    emailLabel.grid(row=3, column=0, padx=6, pady=2, sticky=W)
    emailEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    emailEntry.grid(row=3, column=1, pady=2, padx=2)

    genderLabel = Label(screen, text='Gender', font=('times new roman', 12, 'bold'))
    genderLabel.grid(row=4, column=0, padx=6, pady=2, sticky=W)
    genderEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    genderEntry.grid(row=4, column=1, pady=2, padx=2)

    
    fathernameLabel = Label(screen, text='fathername', font=('times new roman', 12, 'bold'))
    fathernameLabel.grid(row=5, column=0, padx=6, pady=2, sticky=W)
    fathernameEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    fathernameEntry.grid(row=5, column=1, pady=2, padx=2)

    fatheroccupationLabel = Label(screen, text='fatheroccupation', font=('times new roman', 12, 'bold'))
    fatheroccupationLabel.grid(row=6, column=0, padx=6, pady=2, sticky=W)
    fatheroccupationEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    fatheroccupationEntry.grid(row=6, column=1, pady=2, padx=2)

    fatherannualincomeLabel = Label(screen, text='fatherannualincome', font=('times new roman', 12, 'bold'))
    fatherannualincomeLabel.grid(row=7, column=0, padx=6, pady=2, sticky=W)
    fatherannualincomeEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    fatherannualincomeEntry.grid(row=7, column=1, pady=2, padx=2)

    officeadressEntryLabel = Label(screen, text='Father Address Entry', font=('times new roman', 12, 'bold'))
    officeadressEntryLabel.grid(row=7, column=0, padx=6, pady=2, sticky=W)
    officeadressEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    officeadressEntry.grid(row=7, column=1, pady=2, padx=2)


    fatherphonenoLabel = Label(screen, text='father phone no', font=('times new roman', 12, 'bold'))
    fatherphonenoLabel.grid(row=8, column=0, padx=6, pady=2, sticky=W)
    fatherphonenoEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    fatherphonenoEntry.grid(row=8, column=1, pady=2, padx=2)

    mothernameLabel = Label(screen, text='mothername', font=('times new roman', 12, 'bold'))
    mothernameLabel.grid(row=9, column=0, padx=6, pady=2, sticky=W)
    mothernameEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    mothernameEntry.grid(row=9, column=1, pady=2, padx=2)

    motherphonenoLabel = Label(screen, text='mother  phone no', font=('times new roman', 12, 'bold'))
    motherphonenoLabel.grid(row=10, column=0, padx=6, pady=2, sticky=W)
    motherphonenoEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    motherphonenoEntry.grid(row=10, column=1, pady=2, padx=2)

    motheroccupationLabel = Label(screen, text='mother occupation', font=('times new roman', 12, 'bold'))
    motheroccupationLabel.grid(row=11, column=0, padx=6, pady=2, sticky=W)
    motheroccupationEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    motheroccupationEntry.grid(row=11, column=1, pady=2, padx=2)

    motherannualincomeLabel = Label(screen, text='mother annual income', font=('times new roman', 12, 'bold'))
    motherannualincomeLabel.grid(row=12, column=0, padx=6, pady=2, sticky=W)
    motherannualincomeEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    motherannualincomeEntry.grid(row=12, column=1, pady=2, padx=2)
    ...
    if command == "ADD":
        student_button = ttk.Button(screen, text=button_text, command=add_data)
    elif command == "SEARCH":
        student_button = ttk.Button(screen, text=button_text, command=search_data)
    elif command == "UPDATE":
        student_button = ttk.Button(screen, text=button_text, command=update_data)
...
def update_data():
    ...
    con.commit()
    messagebox.showinfo('Success',f'admission no is modified successfully',parent=screen)
...
def add_data():
    global screen
    print(nameEntry,emisnoEntry,emailEntry,genderEntry,fathernameEntry,fatheroccupationEntry,fatherannualincomeEntry,officeadressEntry,fatherphonenoEntry,mothernameEntry,motherphonenoEntry,motheroccupationEntry,motherannualincomeEntry)
    variableDesider = nameEntry.get()=='' or emisnoEntry.get()=='' or emailEntry.get()=='' or genderEntry.get()=='' or fathernameEntry.get()=='' , fatheroccupationEntry.get()=='' or fatherannualincomeEntry.get()=='' or officeadressEntry.get()=='' or fatherphonenoEntry.get()=='' or mothernameEntry.get()=='' or motherphonenoEntry.get()=='' or motheroccupationEntry.get()=='' or motherannualincomeEntry.get()==''
    print("---")
    #if variableDesider:
...
logoimage=PhotoImage(file=r"graduates.png")
logolabel=Label(leftframe,image=logoimage)
logolabel.grid(row=0,column=0,pady=10)

addstudentbutton=ttk.Button(leftframe,text="add student details",command=lambda :toplevel_data('Add Student','Add','ADD'))
addstudentbutton.grid(row=1,column=0,pady=20,padx=80)

searchstudentbutton=ttk.Button(leftframe,text="search student details",command=lambda :toplevel_data('Search Student','Search',"SEARCH"))
searchstudentbutton.grid(row=2,column=0,pady=20,padx=80)


updatestudentbutton=ttk.Button(leftframe,text="update student details",command=lambda :toplevel_data('Update Student','Update',"UPDATE"))
